...
float objectSpaceToInteriorSpace(float f)
{
	return 2 * f - 1;
}
...
void main()
{
	...
	// on [-1, 1]
	float yTop = objectSpaceToInteriorSpace(ceil(roomCountContinuous) * RoomHeightInObjectSpace);
	float yBottom = objectSpaceToInteriorSpace(floor(roomCountContinuous) * RoomHeightInObjectSpace);
...
//	float xIntersectionT = abs(1/eye.x) - (TexCoord.x/eye.x);
	...
	vec3 textureCoord = vec3(
		intersectPoint.x,
		intersectPoint.y,
//		2 * (intersectPoint.y - yBottom)/RoomHeightInObjectSpace - 1,
		intersectPoint.z
	);
	...
	// so that the walls don't jump when we turn a corner
	{
		...
		textureCoord = rot * intersectPoint;
	}
